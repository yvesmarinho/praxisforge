...
### User Story 3 - Levar o registro existente para o novo local (Priority: P3)

O curador que já tem um registro populado dentro do repositório (`src/data/folders.yaml`) move
esse registro para o novo local sem perder nenhuma pasta e sem apagar um registro que já exista
no destino.
...
procedimento de migração; o destino passa a conter as mesmas pastas e o comando seguinte lê o
novo local. Com o destino já existente, o procedimento recusa e nada é sobrescrito.

**Acceptance Scenarios**:

1. **Given** um registro populado em `src/data/folders.yaml` e nenhum registro no local padrão, **When** o curador roda um comando de consulta, **Then** o erro de registro ausente também informa que existe um registro antigo no repositório e como movê-lo.
2. **Given** o mesmo cenário, **When** o curador executa a migração, **Then** o registro passa para o local padrão com conteúdo idêntico e deixa de existir dentro do repositório.
3. **Given** já existe um registro no local padrão, **When** o curador executa a migração, **Then** a migração é recusada, nenhum arquivo é alterado e a mensagem cita os dois locais.

---

### Edge Cases

- Diretório de configuração do usuário definido mas sem permissão de escrita: `add`/`bootstrap` falham com mensagem de ambiente citando o local, sem traceback.
- Local indicado aponta para um diretório (não arquivo): erro de uso citando o local.
...
- Registro antigo no repositório vazio (`folders: {}`): não é tratado como registro a migrar.
...
- Execução a partir de outro diretório de trabalho: o local padrão independe do diretório atual.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: O local do registro MUST ser resolvido nesta ordem de precedência: opção `--registry`; variável `PRAXISFORGE_REGISTRY` (quando não vazia); `$XDG_CONFIG_HOME/praxisforge/folders.yaml` (quando a variável de configuração do usuário estiver definida e não vazia); `~/.config/praxisforge/folders.yaml`.
- **FR-002**: O local resolvido MUST ter `~` expandido e caminho relativo resolvido a partir do diretório atual antes do uso.
- **FR-003**: `folders add` e `folders bootstrap` MUST criar a pasta do registro quando ausente, ao criar o registro.
- **FR-004**: Os demais comandos de pastas MUST falhar com o erro de registro ausente já existente, citando o local resolvido, sem criar nada.
...
- **FR-006**: O repositório MUST versionar `src/data/folders.example.yaml`, um registro válido no contrato vigente, com pastas de exemplo sob caminhos genéricos (sem diretório de usuário).
...
- **FR-008**: Quando o registro não existir no local resolvido e existir um registro antigo não vazio em `src/data/folders.yaml` (a partir do diretório atual), a mensagem de erro MUST também indicar o registro antigo e o procedimento de migração.
- **FR-009**: A migração MUST mover o registro antigo para o local resolvido preservando o conteúdo byte a byte, criando a pasta de destino se preciso, e MUST recusar sem alterar nada quando já houver registro no destino.
- **FR-010**: A migração MUST validar o registro antigo contra o contrato vigente antes de mover; registro inválido MUST ser recusado com a lista de violações.
- **FR-011**: A documentação de operação MUST descrever o local padrão, a precedência, a variável, o exemplo versionado e a migração.
...
- **Registro antigo**: `src/data/folders.yaml` de clones anteriores a esta feature; só existe na máquina do curador após a mudança.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: Com um registro pessoal populado no local padrão, 100% dos testes e da validação de dados passam localmente.
- **SC-002**: O conteúdo versionado não contém nenhum caminho de diretório de usuário (verificado pelo guarda automático).
- **SC-003**: O curador migra um registro de 50+ pastas para o novo local com um único comando, sem perder nenhuma pasta (comparação de conteúdo idêntica).
- **SC-004**: Em 100% dos casos de registro ausente, a mensagem informa o local resolvido e o próximo passo.
- **SC-005**: Um clone novo do repositório roda todos os gates sem nenhuma configuração adicional.

## Assumptions

- Plataforma alvo é Linux (constituição); a convenção de configuração do usuário segue a especificação XDG com o fallback `~/.config`.
- A migração é um comando da CLI (mover arquivo), não uma conversão de formato: o registro antigo já está no contrato v2.

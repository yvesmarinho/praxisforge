...
- [ ] T010 [P] Escrever `tests/contract/test_folders_schema.py`: valida `schemas/folders-schema-v1.json` (metaschema) e casos válidos/inválidos da spec (sem `schema_version`, versão `"2"`, campo ausente, alias maiúsculo, alias que é caminho absoluto, licença `unknown` com status ≠ `pending`, `not_scanned` com data, status fora do conjunto). Inclui `last_scanned` com formato inválido (`"ontem"`, `"2026-13-40T00:00:00-03:00"`) rejeitado (format `date-time` efetivo).
...
- [ ] T024 Implementar `src/praxisforge/infrastructure/jsonschema_validator.py` (Draft 2020-12, `iter_errors` com `format_checker=FormatChecker()` para tornar `date-time`/`date` efetivos, ordena violações por campo) — faz T013 passar
...
- [ ] T027 Rodar `make lint && make test` e `make validate-data` (pulando `folders.yaml` ainda inexistente) — tudo verde, cobertura ≥ 90%. **Recomendado neste ponto:** executar T061–T064 (guarda de camadas, US4), que não dependem de US1–US3, para que toda implementação seguinte seja validada pelo teste de arquitetura
...
- [ ] T034 [US1] Implementar `src/praxisforge/infrastructure/yaml_folder_registry.py` (`safe_load`/`safe_dump` ordenado por alias, escrita atômica com arquivo temporário + `os.replace`, valida com `ContractValidator` ao carregar, `load_raw`, criação do arquivo só via `save` do `add`) — faz T028 passar Carregar com um `SafeLoader` próprio **sem** o resolvedor implícito de timestamp (datas como `last_scanned` permanecem `str` ISO 8601, pois `yaml.safe_load` converteria para `datetime`/`date` e o schema exige string) e gravar datas como string entre aspas; `load_raw()` usa o mesmo loader.
...
- [ ] T047 [US2] Implementar `src/praxisforge/application/resolve_folder_path.py` (`resolve` de um alias e `resolve_all` em lote com `BatchReport`) — faz T042 passar
- [ ] T048 [US2] Adicionar o subcomando `folders resolve (ALIAS | --all)` em `src/praxisforge/presentation/cli.py` (única saída que imprime caminho absoluto) — faz T043 passar
...
- [ ] T052 [P] [US3] Escrever `tests/integration/test_source_frontmatter.py`: lê frontmatter YAML de `.md` em `tmp_path`; sem frontmatter, frontmatter corrompido, arquivo ilegível → erros específicos; lote de arquivos com falha por item Datas do frontmatter (`date: 2026-09-21`) permanecem `str`.
...
- [ ] T057 [P] [US3] Implementar `src/praxisforge/infrastructure/source_frontmatter.py` (leitor de frontmatter, sem executar código, `yaml.safe_load`) — faz T052 passar Usar o mesmo `SafeLoader` sem resolvedor de timestamp de T034 (`date` do frontmatter permanece `str`).
...
- [ ] T067 [P] Criar ADRs em `docs/decisions/` (MADR simplificado): `0001-domain-sem-pydantic.md`, `0002-registro-yaml-sem-preservar-comentarios.md`, `0003-verificacao-de-camadas-por-ast.md`, `0004-update-aceita-licenca.md` O ADR 0003 registra a exceção declarada: o ponto de composição `presentation/cli.py` importa Infrastructure.
...
- [ ] T069 Criar `.pre-commit-config.yaml` (verificações manuais, conforme regra global) com hooks locais `ruff check`, `ruff format --check` e `mypy` e o `ggshield secret scan pre-commit` (chave lida de `.secrets/`/ambiente); rodar `pre-commit install`; criar `.github/workflows/quality-gates.yml` (`setup-uv`, `uv sync`, `make lint && make test && make validate-data && make security`), bloqueante no PR
- [ ] T070 Rodar todos os gates: `uv run ruff check .`, `uv run ruff format --check src tests`, `uv run mypy` (0 erros), `uv run pytest` (cobertura ≥ 90%, incluindo cenários de falha de FS e ambiente — SC-007), `make validate-data`, `make security`
- [ ] T071 Executar o `quickstart.md` completo (§1–§7) e confirmar cada resultado esperado; registrar divergências em `docs/bugs/` se houver (relatório de erro + correção)
- [ ] T072 Atualizar `graphify` (`graphify update .`) e registrar no vault `claude_memory` a feature concluída: `projects/praxisforge.md` (decisões, catálogo, pendências) e `daily/AAAA-MM-DD.md`, listando novas notas no `00-index.md`; sem segredos nem caminhos internos
